...
def make_dataset_custom(root):
    imgs=[]
    for filename in os.listdir(root):
        form = filename.split('_')[-1]
        if 'img' in form:
            name = filename.split('_img')[0]     
            img = os.path.join(root, filename)
            mask1 = os.path.join(root, name + '_ol' + '.JPG')
            mask2 = os.path.join(root, name + '_nonol' + '.JPG')
            imgs.append((img,mask1,mask2))
    print(imgs[:2])
    return imgs

class Dataset(Dataset):
    def __init__(self, root, transform=None, target_transform=None, size=256):
        imgs = make_dataset_custom(root)
        self.imgs = imgs
    ...
    def __getitem__(self, index):
        ...
        if width != self.size or height != self.size:
            img_x = img_x.resize((self.size, self.size))
            img_y1 = img_y1.resize((self.size, self.size))
            img_y2 = img_y2.resize((self.size, self.size))
        
        # print("img_x: ", (np.array(img_x)).shape)
        # print("img_y1: ", (np.array(img_y1)).shape)
        # print("img_y2: ", (np.array(img_y2)).shape)
