...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='original_dataset')
    parser.add_argument('--batch_size', type=int, default=8)
    args = parser.parse_args()
    assert args.model_type in ['UNet', 'UNet_plus', 'UNet_plus2']
    assert args.dataset in ['dataset', 'original_dataset']
    
    
    train_data_dir = "/home/truong/datadrive2/project/ChromSeg/dataset/train/data"
    val_data_dir = "/home/truong/datadrive2/project/ChromSeg/dataset/validate/data"

    # train_data_dir = "/home/truong/datadrive2/project/ChromSeg/region_guided_UNetplus/original_dataset/train"
    # val_data_dir = "/home/truong/datadrive2/project/ChromSeg/region_guided_UNetplus/original_dataset/val"
    
    if args.dataset == 'dataset' or args.dataset == 'original_dataset':
        IMAGE_SIZE = 256
        batch_size = 8
    else:
        print("wrong")
    ...
    dataset_train = Dataset(train_data_dir, transform=x_transforms, target_transform=y_transforms)
    dataloader_train = DataLoader(dataset_train, batch_size=args.batch_size, shuffle=True, num_workers=2)
    dataset_val = Dataset(val_data_dir, transform=x_transforms, target_transform=y_transforms)
    dataloader_val = DataLoader(dataset_val, batch_size=args.batch_size, shuffle=False, num_workers=2)
    
    model = train_model(model, criterion, optimizer, dataloader_train, dataloader_val, num_epochs=100, patience=5)
    ...
    torch.save(model.state_dict(), './models/0530_%s_%s_seed%s_IoU-%0.4f_IoU-%0.4f_gamma-%d_alpha-%0.2f.pth' % (args.model_type, args.dataset, args.seed, IoU, IoU2, gamma, alpha))